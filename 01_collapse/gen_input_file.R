...
if (indi_fam == 'water') {
  ...
  w_network_cr <- mutate(w_network_cr, point = 1, weight = 1, w_network_cr = (network*N*piped), N = N*piped)
  w_network_cr <- rename(w_network_cr, country = iso3, year = year_start, latitude = lat,
...
if (indi_fam == 'sani') {
  ...
  s_network_cr <- mutate(s_network_cr, point = 1, weight = 1, s_network_cr = (network*N*piped), N = N*piped)
  s_network_cr <- rename(s_network_cr, country = iso3, year = year_start, latitude = lat,
                         longitude = long) %>% dplyr::select(-imp,-piped,-network)
  ...
  s_imp_cr <- mutate(s_imp_cr, point = 1, weight = 1, s_imp_cr = (imp*N), s_piped = (piped*N))
  s_imp_cr <- rename(s_imp_cr, country = iso3, year = year_start, N = N, latitude = lat,
                     longitude = long)
