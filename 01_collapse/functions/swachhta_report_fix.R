...
# Read in data
mydat <- fread('/ihme/limited_use/LIMITED_USE/LU_GEOSPATIAL/collapsed/wash/tabulated_data/sani.csv')

sb <- filter(mydat, survey_series %in% c('SWACHHTA_REPORT', 'NARSS')) %>%
  dplyr::select(survey_series, shapefile, location_code, imp_cw, iso3, nid, lat, long, total_hh, 
                year = year_start)

# Read in definitions
setwd('/home/j/WORK/11_geospatial/wash/definitions')

sani_cw <- fread('cw_sani_2_ani_use_this.csv')
...
cw_ratios <- ind_cw %>%
  ...
  mutate(
    s_network = network,
    s_piped = s_piped,
    s_imp_other = imp + latrine_imp + latrine_cw*(latrine_imp/(latrine_imp+latrine_unimp)),
    s_unimp = unimp + latrine_cw*(latrine_unimp/(latrine_unimp+latrine_unimp)) + latrine_unimp +
      flush_cw*(flush_unimp/(flush_unimp+flush_imp)) + flush_unimp,
    s_od = od
  ) %>%
